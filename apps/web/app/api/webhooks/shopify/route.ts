...
export async function POST(req: NextRequest) {
  ...
  if (webhookId && redis) {
    try {
      ...
      console.warn('[Shopify Webhook] Redis idempotency check failed, continuing:', err);
    }
  }
  ...
  try {
    ...
    if (!conn) {
      ...
      conn = allConnections.find(
        (c) => c.shopDomain?.toLowerCase() === normalizedShop.toLowerCase(),
      ) || null;
    }
    
    if (!conn) {
      // If no connection found for this shop, skip persistence safely
      console.warn('[Shopify Webhook] No connection found for shop:', normalizedShop, 'topic:', topic);
      await logEvent('shopify.webhook.no_connection', { shop: normalizedShop, topic });
    ...
    if (topic === 'orders/create') {
      ...
      console.log('[Shopify Webhook] Order saved successfully:', String(order.id));
  ...
  if (webhookId && redis) {
    try {
      ...
      console.warn('[Shopify Webhook] Failed to set Redis key, continuing:', err);
    }
  }
