...
async function saveLineItems(
  ...
  try {
    ...
    console.log('[Shopify Webhook] Line items saved:', {
      orderId,
      count: lineItems.length,
    });
  } catch (err) {
    console.error('[Shopify Webhook] Failed to save line items:', {
      orderId,
      error: err instanceof Error ? err.message : String(err),
    });
    // Don't fail the webhook - line items are supplementary
  }
}
