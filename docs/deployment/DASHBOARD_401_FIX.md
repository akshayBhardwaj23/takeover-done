# Fix 401 Error When Sending Emails from Dashboard
...
1. **Improved error handling** in dashboard to show actual error message
2. **Added logging** in tRPC handler to show session status
3. **Better error messages** to distinguish between:
   - `UNAUTHORIZED` → Not logged in
   - `FORBIDDEN` → Email limit reached
   - Other errors → Actual error message shown

## Next Steps

1. **Check if you're logged in** (top-right corner)
2. **Open browser console** and try sending email
3. **Check the error message** - should now be more specific
4. **Check Vercel logs** for session details
5. **If still 401:**
